...
| ActivitiesSearchSkill | Shared / Personal（feature flag） | Shared：snapshot-bound destination → normalized Activities evidence 或 `UNAVAILABLE`；Personal：仅 server-owned owner/thread/trip context，结果留在 owner conversation 与独立 Personal evidence 表。 | ActivitiesProvider、Amadeus Tours & Activities adapter、`ActivityDestinationReference`、per-endpoint limiter。 | 目的地坐标/半径仅由 server reference 解析；theme 为固定 allow-list；不成功不生成替代 offer；Personal 结果永不进入 Shared context/snapshot/plan；booking link 被 adapter 丢弃。 |
