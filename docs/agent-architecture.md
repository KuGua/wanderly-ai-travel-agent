...
所有展示给用户的自然语言内容必须遵循同一语言优先级：当前轮明确要求的翻译/指定语言优先；否则使用当前轮用户输入的主要语言。历史对话、长期记忆、目的地所在国家和 provider evidence 只能提供语境，不能改变该选择；专有名词可保留常用或当地写法。

该契约不适用于 JSON、证据、工具参数、ID、枚举及其他机器消费的输出，它们必须保持各自的类型和事实边界。没有当前用户输入的公共可缓存内容，以服务端验证的 `locale` 为语言权威；例如地点介绍当前只支持 `en` / `zh`，并按 locale 分开缓存。
