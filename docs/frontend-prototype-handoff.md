...
2. **来源与不可用优先。** 航班、住宿、地面交通和 readiness 的展示均需要 `source` + `captured_at`；无可验证数据时显示 `UNAVAILABLE`，不得显示 Demo data 替代旅行事实。地图和视觉原型 fixture 仅限无事实承诺的展示层。
...
| 类型 | 数据 | 抽屉内容 | CTA |
|---|---|---|---|
| 固定候选 | 服务端候选 | 已授权匹配原因、来源/时间、报价有效期或 `UNAVAILABLE` 缺口 | `查看候选方案` / `围绕此地规划` |
...
- “继续出发”使用比普通卡更高的强调卡：左侧为不携带事实的路线抽象图，右侧呈现状态、项目名、说明、成员数量、日期、来源/不可用状态和唯一下一步。
...
| 私人探索草稿 | 有至少一条私有对话、尚未显式激活的 `DRAFT` 项目 | `私人探索草稿` | 打开与进行中项目相同的项目工作台，继续私有对话并在对话中确认 brief |
...
- 无项目：显示“还没有旅行项目”与 `从地图开始探索` / `新建规划` 两个入口。仅浏览地图不会创建项目；用户首次发送探索消息后才出现私人探索草稿。
...
type ProjectSummary = {
  id: string;
  name: string;
  tripStatus: "DRAFT" | "PLANNING" | "CONFIRMED" | "BOOKED" | "CANCELLED" | "STALE";
