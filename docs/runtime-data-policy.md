...
Amadeus Tours & Activities 与 Amadeus Flight Offers Search 共享 OAuth client-credentials 与请求 deadline，但两个工具是相互独立的 typed port：分别拥有独立的 provider adapter、覆盖矩阵、stale 触发器、evidence 写入、不可用语义与 audit action。两者必须以 per-endpoint TPS 限流隔离分配，以免任一方耗尽共享配额导致另一方连带失败。失败、超时、限流、空数据统一返回 `UNAVAILABLE`，不创建 plan、offer、source evidence、预订参考号或签证资格结论，也不得以 fixture、Demo data 或模型编造的内容替代。
