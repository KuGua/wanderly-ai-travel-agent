...
在匹配到国家时，返回 `REFERENCE`、可选省/州和可选最近城市。命中城市时同时返回 `nearestCityCoordinates`，其值是同一条 GeoNames 索引记录的中心点；前端只可用它把当前会话图钉归一到城市中心和做同城去重，不能把它解释成行政边界、精确地址或导航点。最近城市超过 75 km 时，城市名称、中心点与距离必须同时为 `null`，不能把“最近城市”称作行政归属。海洋、未覆盖区域返回 `NO_REFERENCE`；超过匿名限流返回 `429`；数据文件无法读取或验证时返回 `503`，不推断结果。
