...
`POST /api/v1/explore/location-reference` 将用户**明确点击**的坐标解析为只读的地图位置参考。它不是地址服务、旅行 provider 或业务真相：结果不能创建目的地候选、私有灵感持久化、共享约束、plan、价格、库存、签证或预订结论。
...
在匹配到国家时，返回 `REFERENCE`、可选省/州和可选最近城市。最近城市超过 75 km 时必须省略，不能把“最近城市”称作行政归属。海洋、未覆盖区域返回 `NO_REFERENCE`；数据文件无法读取或验证时返回 `503`，不推断结果。
