...
5. Submit malformed model output through `PlanningService` and inspect persistence and the API error.
...
- A failed candidate creates no `itineraryPlans`, `providerOffers`, `sourceEvidence` or plan audit record.
