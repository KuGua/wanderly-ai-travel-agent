...
- 下方集合从中段开始，向左或向右连续滚动都无端点、空白或可见倒放；越过半段后按一个完整段宽无感回绕。视觉副本不进入读屏与 Tab 顺序，每个真实项目只被键盘访问一次，链接和项目数据不变；窗口尺寸变化后仍保持循环。
...
### TS-RESEARCHED-EVIDENCE-REACHES-PLAN — 撤下工具必须同时交接证据

**Objective:** `TS-NO-REOFFER-RESEARCHED-TOOLS` 把已研究能力的工具从模型手里撤下，
但没有把它们的结果交给合成。`allHotels` / `allActivities` / `allAccommodations`
**唯一的写入点就是那些工具的派发处理器**，撤掉工具＝撤掉唯一写入路径。结果是一轮
握着 10 条 Nuitee 报价、16 条住宿、4 条活动的运行，产出的方案只引用了航班，
并且仍然报 `stay: NO_RESULTS`。

「酒店」这一格另有三处独立缺陷，四条必须同批修，否则只修交接会把"空卡片"变成
"运行失败"（`requireHotels` 在有酒店证据时**要求**方案含 `hotels`）。
...
**Objective:** Verify that `POST /trips/:tripId/activate` accepts both shapes of
`quoteNationalityDecision`, and that request validation never rewrites the body
it is validating.

**Starting conditions:** Alice owns a solo `DRAFT` Trip with a complete brief
(departures, destinations, start and end dates). The quote-nationality cipher is
a reversible test double. Alice's Profile has no stored nationality unless a
step says so.

**Steps:**

1. Activate with `quoteNationalityDecision: { source: "INPUT", value: "TW",
   saveToProfile: false, confirmProviderUse: true }`.
2. Activate a second Draft with the same decision and `saveToProfile: true`,
   using a lower-case `value`.
3. Store a Profile nationality, then activate a third Draft with
   `{ source: "PROFILE", confirmProviderUse: true }`.
4. With no Profile nationality stored, activate with `{ source: "PROFILE",
   confirmProviderUse: true }`.
5. Activate with a misspelled brief field (e.g. `travelDatesEnd`) alongside an
   otherwise valid body.

**Expected outcomes:**

- Steps 1–3 return `200` and the Trip reaches `PLANNING`. Each leaves exactly
  one ACTIVE `nuitee_connect` / `guest_nationality` authorization for
  `(trip, member)` holding the normalized value (`TW`).
- Step 1 writes no `user_profiles` row: authorizing the provider call is not
  consent to store the field. Step 2 stores `TW` on the Profile.
- Step 4 fails closed with `422` and leaves the Trip in `DRAFT`; the server
  never guesses a nationality it was not given.
- Step 5 is rejected with `400` and leaves the Trip in `DRAFT` — an unknown
  field is refused, not silently discarded and activated around.
- The nationality value appears in no audit summary, log field, metric label or
  response body on any of these paths.

**Regression guarded:** Fastify's default AJV (`removeAdditional: true`) edits
the body in place while validating. Inside `quoteNationalityDecision`'s `oneOf`
the PROFILE branch's `additionalProperties: false` deleted `value` and
`saveToProfile` before the INPUT branch was evaluated, so every hand-entered
nationality was answered `400 must have required property 'value'` and a
traveller without a stored nationality could never leave `DRAFT`. The same
option was also swallowing unknown fields before each schema's `.strict()`
could refuse them.
