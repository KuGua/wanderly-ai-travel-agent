...
14. DRAFT 私有对话中的 destination brief proposal 仅可来自 owner 在当前轮明确设置的行程表达，或服务端已核验的地图地点；助手回复中提及的地点、解释性问题、代词、未知文本和歧义地点不得形成候选。服务端必须把每个候选解析为唯一 `DestinationReference` 后才展示确认卡，并在 creator 确认写入 DRAFT brief 时再次解析并规范化；无法解析时 fail closed，返回可恢复错误且不改写现有 brief。模型可协助解释 owner 对本轮具体日期/天数提议的接受，但不得单独创建 destination 或 departure 事实。
