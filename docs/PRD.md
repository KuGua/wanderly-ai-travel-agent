...
`/projects` 是全屏阳光像素唱片室：旋转唱片对应当前账号、本标签页最近成功打开且
仍有权限的 Trip，其他真实行程横向排列为方形纸板唱片套，聚焦/悬停时抬起并抽出黑胶。
墙上显示只读的本地当前月挂历，标记今天，不映射行程状态。点击唱片进入项目。保留侧边栏，不显示
年历或筛选面板；桌面垃圾桶支持将创建者自己的唱片拖入（或点击选择），明确确认后永久删除行程及其关联内容，服务端校验创建者权限；右上角「新增行程」以幂等请求创建 Draft 并进入项目，
未登录不提供创建操作；探索对话仍可创建。旋转不是任务状态，不播放
音频，不读取 Profile。无访问记录时留空唱盘，不用创建时间猜测最近打开的项目。
