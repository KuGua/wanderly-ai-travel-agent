...
14. DRAFT creator 的目的地确认由独立 Destination Cue 模型判断；模型只接收当前 USER 消息、已确认目的地和 locale，不接收 Assistant、历史或地图选中状态。普通机票与酒店查询不得触发 Cue，但同句明确“把 X 设为目的地”的指令优先。所有候选必须由服务端解析为唯一城市，按 owner/thread 持久化并逐项接受或忽略；多地点箭头仅切换，处理一项不得关闭整批。刷新从 conversation REST 恢复，SSE 只作即时通知。忽略按地点执行 30 分钟 + 2 次后续合格提及策略，24 小时静默后清零。日期、天数、出发地继续使用独立 Trip Brief review；模型可协助解释 owner 对本轮具体日期/天数提议的接受，但不得单独创建 destination 或 departure 事实。实施合同见 [Destination Cue Decision Model](destination-cue-model-draft.md)。
