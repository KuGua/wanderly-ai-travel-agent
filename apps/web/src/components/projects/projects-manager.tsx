...
export function ProjectsManager() {
  ...
  return <main className={styles.room} aria-label={t("room")}>
    <div className={styles.scene}>
      ...
      <WallCalendar />
      <div className={styles.collection} aria-label={t("collection")}>
        {others.map((trip) => <Link key={trip.id} href={`/trips/${trip.id}` as "/trips/[tripId]"} className={styles.sleeve} aria-label={t("open", {name:trip.name})} title={trip.name} style={{"--label":recordColor(trip.id)} as CSSProperties}>
