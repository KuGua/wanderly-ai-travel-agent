...
import { useLayoutEffect, useRef, useState, useSyncExternalStore, type CSSProperties, type UIEvent, type DragEvent, type KeyboardEvent } from "react";
...
export function ProjectsManager() {
  ...
  const { collectionRef, segmentRef, onScroll } = useLoopingCollection(others.length);
  const [dragging, setDragging] = useState<{ viewer: string; id: string } | null>(null);
  const [deletion, setDeletion] = useState<{ viewer: string; id: string } | null>(null);
  const recordActions = (trip: TripSummary) => ({
    draggable: trip.role === "CREATOR",
    onDragStart: (event: DragEvent<HTMLAnchorElement>) => {
      if (!viewer || trip.role !== "CREATOR") { event.preventDefault(); return; }
      event.dataTransfer.effectAllowed = "move";
      event.dataTransfer.setData("application/x-wanderly-record", trip.id);
      setDragging({ viewer, id: trip.id });
    },
    onDragEnd: () => setDragging(null),
    onKeyDown: (event: KeyboardEvent<HTMLAnchorElement>) => {
      if (event.key === "Delete" && viewer && trip.role === "CREATOR") {
        event.preventDefault(); setDeletion({ viewer, id: trip.id });
      }
    },
  });
  const recordSet = (copy: "before" | "main" | "after") => <div
    key={copy}
    ref={copy === "main" ? segmentRef : undefined}
    className={styles.collectionSet}
    aria-hidden={copy === "main" ? undefined : true}
  >
    {others.map((trip) => <Link key={`${copy}-${trip.id}`} {...recordActions(trip)} href={`/trips/${trip.id}` as "/trips/[tripId]"} className={styles.sleeve} aria-label={t("open", {name:trip.name})} title={trip.name} tabIndex={copy === "main" ? undefined : -1} style={{"--label":recordColor(trip.id)} as CSSProperties}>
  ...
  return <main className={styles.room} aria-label={t("room")}>
    <div className={styles.scene}>
      ...
      <WallCalendar />
      {viewer && <RecordTrash key={viewer} trips={trips} draggedId={dragging?.viewer === viewer ? dragging.id : null}
        selection={deletion?.viewer === viewer ? deletion.id : null}
        onSelect={(id) => setDeletion(id === null ? null : { viewer, id })} onDragClear={() => setDragging(null)} />}
      <div ref={collectionRef} className={styles.collection} aria-label={t("collection")} onScroll={onScroll}>
