...
export function TravelAgentChat({ onOpenChange, selectedPlace }: TravelAgentChatProps) {
  ...
  const conversation = (
    <aside role="dialog" aria-label="Wanderly Agent conversation" data-expanded={expanded ? "true" : "false"} className={`flex flex-col overflow-hidden shadow-[0_28px_90px_rgb(8_47_63/28%)] transition-[inset,height,border-radius] duration-300 ${expanded ? "fixed inset-0 z-[100] h-dvh rounded-none" : "absolute inset-x-3 bottom-3 z-50 h-[43dvh] min-h-[300px] rounded-[28px] md:inset-y-6 md:left-auto md:right-6 md:h-auto md:min-h-0 md:w-[min(420px,calc(100%-2rem))]"}`}>
      <div className={`flex min-h-0 flex-1 flex-col overflow-hidden border-x border-t border-white/80 bg-white md:border ${expanded ? "rounded-none" : "rounded-t-[28px] md:rounded-[28px]"}`}>
        <header className="relative flex items-center gap-3 border-b border-[#dbe8e5] px-5 pb-4 pt-5">
          ...
          <span className="grid size-10 place-items-center rounded-[14px] bg-sidebar text-white shadow-sm">
            <MessageCircle aria-hidden="true" className="size-5" />
          </span>
          <div className="min-w-0 flex-1">
            <p className="font-black tracking-[-0.025em] text-sidebar">Wanderly Agent</p>
            <p className="text-xs text-muted-foreground">Private travel conversation</p>
          </div>
          <button type="button" onClick={closeConversation} aria-label="Close conversation" className="grid size-10 place-items-center rounded-full bg-sidebar text-white transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sidebar/25">
