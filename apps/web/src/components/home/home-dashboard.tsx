  ...
  return (
    <main className="mx-auto w-full max-w-[1240px] px-5 py-8 sm:px-8 md:px-[clamp(2rem,4vw,3.5rem)] md:py-[42px]">
      <header className="flex flex-wrap items-start justify-between gap-6">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.11em] text-primary">
            {tHome("kicker")}
          </p>
        ...
        <Link
          href="/home"
          className="inline-flex min-h-12 items-center gap-2 rounded-[15px] bg-sidebar px-5 text-sm font-bold text-white shadow-[0_10px_24px_rgb(239_118_84/28%)] transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sidebar/25"
        >
      ...
      >
        <article className="flex min-h-[116px] items-center gap-4 rounded-[22px] border border-[#a4ddd2] bg-[linear-gradient(110deg,#effbf7,var(--card))] p-[18px] shadow-[0_8px_24px_#102a4308] sm:col-span-2 lg:col-span-1">
          <span
            className="relative grid size-[55px] shrink-0 place-items-center rounded-[18px] bg-[#ef7654] text-xl font-black text-white shadow-[inset_0_-7px_#d85544]"
            aria-hidden="true"
          >
            {tCommon("brandGlyph")}
          </span>
      ...
      <section className="mt-10" aria-labelledby="trips-heading">
          ...
          <div className="flex flex-wrap gap-2" role="group" aria-label={tHome("filter.ariaLabel")}>
            {filters.map(({ key, count }) => (
              <button
                ...
                className={`inline-flex min-h-[39px] items-center gap-1.5 rounded-full border px-3.5 py-[7px] text-sm font-bold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/30 ${
                  filter === key
              ...
              >
                {tHome(`filter.${key}`)}
                <span className={`tabular-nums ${filter === key ? "text-white/80" : "text-muted-foreground/60"}`}>
                  {count}
                </span>
              </button>
            ))}
          </div>
          <label className="flex min-h-[42px] w-full items-center gap-2 rounded-[14px] border bg-card px-3 sm:w-[min(250px,100%)]">
        ...
        {heroTrip && filter === "active" && !searchQuery ? (
          <section className="mb-6" aria-label={tHome("hero.ariaLabel")}>
            ...
            <article className="grid overflow-hidden rounded-[24px] border border-[#f0bf98] bg-card shadow-[0_18px_50px_#102a4320] sm:grid-cols-[170px_minmax(0,1fr)_auto] lg:grid-cols-[200px_minmax(0,1fr)_auto]">
              <div className="relative min-h-[120px] overflow-hidden bg-[linear-gradient(140deg,#0b6574,#073d50_68%,#052e3f)] sm:min-h-[180px]" aria-hidden="true">
                <span className="absolute -left-5 top-[30px] h-[110px] w-[210px] -rotate-[18deg] rounded-[50%] border-2 border-dashed border-[#f6bd60]" />
                <span className="absolute left-[74px] top-[69px] size-[21px] -rotate-45 rounded-[50%_50%_50%_5px] border-[5px] border-white bg-[#ef7654]" />
              </div>
              <div className="p-5 sm:p-6">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-[#fff1ca] px-2.5 py-1 text-xs font-black text-[#9c5400]">
              ...
              <div className="flex items-center justify-center p-5">
                <Link
                  ...
                  className="inline-flex min-h-[45px] items-center gap-2 rounded-[14px] bg-sidebar px-4 text-sm font-bold text-white shadow-md transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sidebar/25"
                >
                  {heroTrip.status === "STALE"
