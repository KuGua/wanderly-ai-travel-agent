...
export function useLocationIntroduction(options: UseLocationIntroductionOptions): LocationIntroductionQuery {
  ...
  const query = useQuery<LocationIntroductionState, Error>({
    ...
    queryFn: async ({ signal }) => {
      ...
      // This callback runs after render. Resetting here keeps retry accounting
      // scoped to the current request without reading or mutating a ref during
      // render when source or locale changes.
