...
async function migrate() {
  ...
  await db.execute(sql`
    DO $$ BEGIN
      CREATE TYPE audit_action AS ENUM ('PROFILE_CREATE', 'PROFILE_UPDATE', 'PROFILE_DELETE', 'TRIP_CREATE', 'TRIP_JOIN', 'CONSENT_GRANT', 'CONSENT_REVOKE', 'PLAN_CREATE', 'PLAN_STALE', 'PLAN_REPLAN', 'CONFIRMATION_SET', 'BOOKING_SUBMIT', 'BOOKING_RESULT', 'CHANGE_EVENT');
    EXCEPTION WHEN duplicate_object THEN null;
    END $$;
  `);
