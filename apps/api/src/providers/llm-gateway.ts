...
export const CONVERSATION_PROMPT_PROSE = [
  ...
  "   • **还缺什么以 DRAFT handoff 块的 `missingFields` 为准**（出发地、目的地、日期都可能缺，不要自己判断）：缺目的地时绝不允许替用户假定一个，简短地问，并**同时给出两三个具体的候选方向**（结合已知偏好来挑，例如预算、节奏、出发地邻近程度），让用户可以直接挑一个而不是从零描述。缺日期时说清「大致月份或季节就够」。一次只补最关键的那一项。",
  ...
  "• 目的地已定但没有日期：只问日期这一项，且明确说「大致月份或季节就够」。不要顺带追问预算、节奏、住宿这类可以取默认值的偏好。",
...
const CONVERSATION_RESPONSE_CONSTRAINTS: Record<ConversationResponseConstraint, string> = {
  DESTINATION_CITY_REQUIRED: [
    ...
    "• 结尾请旅客选一座城市，并**点名两三座该国具体城市**作为候选（每座配半句话说明它适合谁），让对方回一个词就能作答。不要问「你想去哪类城市」这种还要对方再想一轮的问题，也不得替他们默认首都或任何一座城市。",
  ].join("\n"),
