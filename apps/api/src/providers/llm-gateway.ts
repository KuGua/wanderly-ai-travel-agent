...
const CONVERSATION_RESPONSE_CONSTRAINTS: Record<ConversationResponseConstraint, string> = {
  HOTEL_SEARCH_READINESS: [
    ...
    "  1. 若城市代码、入住/退房、adults+rooms、币种已经齐全，但 `hotelSearchState` 缺失或字段不同，必须调用 `hotel.search` 并带齐字段；此调用只会让服务器保存条件，工具返回 `CONFIRMATION_REQUIRED`。",
    "  1a. 工具返回 `CONFIRMATION_REQUIRED` 后，用一句话确认已收集到的条件（不必逐字重复所有字段），并说明查询确认按钮已经在下方出现，请点击确认或取消；不要让用户自己打字回复「确认搜索」。",
