...
const CONVERSATION_RESPONSE_CONSTRAINTS: Record<ConversationResponseConstraint, string> = {
  HOTEL_SEARCH_READINESS: [
    ...
    "• 报价币种的示例必须动态生成、贴合本轮：第一个优先使用用户所在国家/地区的常用货币，第二个使用目的地当地货币；中间用「或」连接，格式严格按「例如 <XXX> 或 <YYY>」写出（例：大陆用户问台北酒店时应写「例如 CNY 或 TWD」）。判断用户所在国家/地区按下列优先级：memoryContext 中显式记录的居住地/国籍 → tripContext.departureCities 所在国家 → question 语言的常见母国 → 兜底使用 USD。目的地当地货币由 place.name 或 threadContext 已锁定的城市确定；用户已显式给出币种时直接使用，不再生成示例。",
    ...
    "  2. 当用户明确表达“继续搜索 / 现在搜索”（典型措辞：「确认搜索」/「确认搜索酒店」/「确认」/「yes, search」/「go ahead」/「执行搜索」/「ok, search」/「do it」/「开始」等）且 `hotelSearchState` 已完整时，必须调用 `hotel.search`，可传 `{}` 复用该服务端状态。",
    "  2a. 若用户说的是「确认搜索机票」这类**点名机票**的确认，那是机票确认按钮送来的，只授权机票搜索——此时不得调用 `hotel.search`。反之「确认搜索酒店」不授权机票搜索。",
