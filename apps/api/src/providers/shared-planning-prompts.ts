...
export const SHARED_STRUCTURED_PLANNING_SYSTEM_PROMPT = [
  ...
  "Return one JSON object with exactly one top-level plan field. The plan must contain destination, flights, and generatedAt. `stays` and `accommodations` are retired final-plan fields and must never be emitted. `hotels` is the only final-plan accommodation selection. Select only supplied evidence and never manufacture facts outside the supplied snapshot.",
  "Each id you select must reference an entry from the SAME category in the supplied catalog or Tool results. accommodation.discover is research-only coverage and its entries must not be copied into the plan. When an optional category produced no selectable evidence, omit it instead of borrowing an entry from another category.",
].join(" ");
...
export const SHARED_TOOL_PLANNING_SYSTEM_PROMPT = [
  ...
  "Each id you select must reference an entry from the SAME category in the supplied catalog or Tool results. `stays` and `accommodations` are retired final-plan fields and must never be emitted. `hotels` is the only final-plan accommodation selection; accommodation.discover is research-only coverage. When an optional category produced no evidence, omit it instead of borrowing an entry from another category.",
  ...
  "In the final plan, flights, activities, and hotels are compact selections: copy only the exact id of each selected evidence item as an object shaped {\"id\":\"...\"}. The server rebinds those ids to authoritative evidence. Never emit the retired `stays` or `accommodations` fields, or an id absent from Tool results and availableEvidence.",
