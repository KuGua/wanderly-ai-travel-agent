...
| `offer_cue_action_total` | counter | capability, action, outcome, source | capability ∈ `["flight","hotel"]`; action ∈ `["accept","dismiss"]`; outcome ∈ `["success","stale_version","not_found","forbidden","trip_state","conflict","expired","failure"]`; source ∈ `["card_button","result_card_button"]` |
| `offer_cue_decision_total` | counter | capability, outcome | capability ∈ `["flight","hotel"]`; outcome ∈ `["created","skipped_policy","skipped_duplicate","skipped_freshness","skipped_clarification","failure","no_candidates","needs_clarification"]` |
| `offer_cue_resolution_total` | counter | capability, outcome | capability ∈ `["flight","hotel"]`; outcome ∈ `["resolved","superseded","expired"]` |
| `offer_cue_resolver_duration_ms` | histogram | capability, outcome | capability ∈ `["flight","hotel"]`; outcome ∈ `["success","timeout","parse_error","upstream_failure","client_unavailable"]`; buckets = `[50,100,250,500,1000,2000,5000,9000,15000,30000]` |
| `personal_offer_selection_total` | counter | capability, outcome | capability ∈ `["flight","hotel"]`; outcome ∈ `["created","superseded","removed"]` |
...
| `trip_title_writes_total` | counter | source, result | source ∈ `["reference","llm","manual"]`; result ∈ `["applied","rejected","unavailable","no_material","manual_locked","not_draft","superseded","rate_limited"]` |
