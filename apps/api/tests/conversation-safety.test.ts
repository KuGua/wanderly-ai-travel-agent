...
describe("conversation operational fact boundary", () => {
  ...
  ])("returns a deterministic fallback before calling the model for a %s question", async (_label, question) => {
  ...
  ])("replaces an unsupported %s model claim with a deterministic fallback", async (_label, unsafeContent) => {
