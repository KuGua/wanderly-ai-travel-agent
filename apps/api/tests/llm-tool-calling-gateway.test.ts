...
describe("LLMGateway planning tools", () => {
  ...
  it("fails without an extra tool dispatch when the turn limit is exhausted", async () => {
    ...
    await expect(gateway.generateStructuredPlanWithTools!({ destination: "NRT", stays: [], memberPreferences: {}, maxTurns: 1, tools: [{ name: "flight.search", description: "test", parameters: {} }], dispatchTool })).rejects.toMatchObject<ModelGatewayError>({ code: "TOOL_CALL_MAX_TURNS" });
    expect(dispatchTool).toHaveBeenCalledOnce();
  });
});
