...
describe("conversational ModelGateway", () => {
  it("streams OpenAI-compatible deltas in order and records only safe telemetry", async () => {
    ...
    expect(messages[0]?.content).toContain("User-visible language (overrides the language of everything else in this prompt)");
    expect(messages[0]?.content.match(/User-visible language \(overrides the language of everything else in this prompt\)/g)).toHaveLength(1);
    expect(messages[0]?.content).toContain("Write EVERY natural-language reply shown to the traveller in English");
    expect(messages[0]?.content).toContain("if the traveller explicitly asks you to answer in another language");
    // Last, not buried among the Chinese rules above it.
    expect(messages[0]?.content.trimEnd().endsWith("Proper nouns and established place or brand names may retain their usual or local spelling.")).toBe(true);
