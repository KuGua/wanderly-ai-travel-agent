...
| DRAFT 目的地确认 | 独立 Destination Cue structured-output 模型 + 服务端机酒硬门禁 + `LocationReferenceResolver` + PostgreSQL owner/thread 状态机 | 只读当前 USER turn；模型与 Assistant 回复并行且失败不阻断聊天。候选逐项确认、request/version 幂等、REST 恢复与 SSE 即时通知复用现有 Fastify/Worker/TanStack Query，不新增队列或数据库。 | 从 Assistant/history/地图选择推断目的地；复用共享 `pending_brief_proposal`；让模型直接写 Trip；为 Cue 新建微服务、向量库或 Redis。 |
| 客户端 | **Next.js + React + TypeScript**，部署到 **AWS Amplify Hosting** | 浏览器链接最适合三人邀请、独立授权、共同查看、投屏与移动端访问。Amplify 支持 Next.js SSR 部署。[AWS Amplify](https://docs.aws.amazon.com/amplify/latest/userguide/ssr-amplify-support.html) | 原生 iOS/Android App、应用商店发布、离线协作。 |
