...
Amazon RDS for PostgreSQL
        │
        ├─ Amadeus / FlightAPI.io / SerpAPI Google Flights (Flight adapters; server-side only)
...
| 身份 | **Amazon Cognito User Pool**，邮箱或手机号登录，API 验证 access token | 身份来自已验证 JWT 的 `sub`，前端不能通过用户 ID 或 demo 角色选择身份。生产使用 Cognito；本地仅允许显式 `local-dev`（固定单一身份 smoke test）或 `custom-local`（数据库用户名/密码和 API JWT，用于多用户隔离测试）。两者仅限 development/test；`local-dev` 必须 server/client loopback，`custom-local` 才可为短期可信内网测试绑定精确 RFC1918 IPv4 host/origin。 | 复杂 SSO、社交登录矩阵、组织管理。 |
...
| 长期记忆 | **PostgreSQL 中结构化、版本化的个人事实 + 当前 Trip 记忆投影 + owner-only Personal Note** | 复用 `user_profiles`、`preference_facts`、字段级 consent、不可变 `constraint_snapshot` 和 stale/replan 控制面。低风险行为只能形成待确认的建议；个人事实默认私有且**完全豁免时间衰减**，Shared Agent 只消费当前 Trip 的最小授权投影。Personal Note 由 Profile 表单显式维护、只提供给 owner Personal Agent 的有界上下文，绝不进入 snapshot、Team 或 planning。建议的排序与触发使用 ACT-R base-level activation 的 Petrov hybrid 近似（`d=0.35`、`k=10`、`τ=0.50`，见 [长期记忆实施方案](docs/long-term-memory-implementation.md) §3.5）；仅保存单字段、单候选、按 UTC day 粗化、上限 10 条的近期观察窗口，不保存行为时间线。 | 向量库、embedding、RAG、独立 memory service、跨 Trip Team memory、从私聊自动写入长期记忆、敏感字段写入 Personal Note。 |
| 工具与模型边界 | Zod schema、structured outputs、server-side policy gate、受限 thread context builder | 对话 archive 仅由所有者读取。Personal Agent 仅可由服务端从同一 owner 的同一私有 thread 构造最近、有预算的原文上下文；该上下文只发送给已配置模型 provider，不进入共享 snapshot、Profile、日志、trace、audit、metric 或客户端持久状态。原文窗口受 `CONVERSATION_CONTEXT_MAX_TURNS`（默认 8 完整轮次，上限 12）和 `CONVERSATION_CONTEXT_MAX_CHARS`（默认 12,000 UTF-16 字符，上限 20,000）双重预算限制，并以 task acceptance 时记录的 `agent_task_runs.context_max_message_sequence` 为不可回写上界。所有共享工具只获得当前 `constraint_snapshot` 的最小授权字段。模型输出不直接成为业务真相。Provider structured-output wire schema 只表达兼容的传输结构；完整长度、格式、日期、时间和证据不变量由独立 canonical Zod schema 与服务端 validator 强制执行，避免供应商 JSON Schema 方言改变业务边界。 | 将整段私聊、其他 thread 或共享/未授权数据放进 prompt；由浏览器提交 history；向量库、Redis 或独立 memory service；自动摘要 worker；tokenizer/embedding；向遥测或前端暴露供应商 key。 |
| DRAFT 确认 Cue | 独立 Destination / Flight Offer / Hotel Offer structured-output 模型 + 服务端 resolver/policy + PostgreSQL owner/thread 状态机 | Destination 只读当前 USER turn；Flight/Hotel 除当前 USER turn 外仅接收服务端投影的、已展示且未过期的私有 offer set。三者均与 Assistant 回复并行，失败不阻断聊天；候选逐项确认、request/version 幂等、REST 恢复与 SSE 即时通知复用现有 Fastify/Worker/TanStack Query，不新增队列、Redis 或微服务。当前卡片呈现顺序为 **Destination → Flight → Hotel**；目的地卡的上/下一个箭头只切换本批目的地候选。Flight/Hotel accept 只保存 owner-only selection，不创建订单、不直接成为 Shared Plan evidence；二者由 `OFFER_CUE_ENABLED` 与能力 allow-list 灰度启用。 | 从 Assistant/history/地图选择或 raw provider payload 推断；以关键词 hard-code 决定文本触发；让模型直接写 Trip/Plan/booking；为 Cue 新建微服务、向量库或 Redis。 |
| 客户端 | **Next.js + React + TypeScript**，部署到 **AWS Amplify Hosting** | 浏览器链接最适合三人邀请、独立授权、共同查看、投屏与移动端访问。Amplify 支持 Next.js SSR 部署。[AWS Amplify](https://docs.aws.amazon.com/amplify/latest/userguide/ssr-amplify-support.html) | 原生 iOS/Android App、应用商店发布、离线协作。 |
| 旅行与数据 API | Amadeus Self-Service Flight Offers Search、FlightAPI.io Flight Price、SerpAPI Google Flights、Viator Experiences MCP、Amadeus Transfer Search；Nuitee Connect / LiteAPI Rates（默认酒店报价）与 SerpApi Google Hotels（可切换）；openrouteservice Place/POI 与 Directions；Frankfurter；通过 typed provider adapters | Flight、Activities、Hotel 保持独立 port。供应商 key 只在服务端 adapter 内使用，完整请求 URL 不进入日志、trace、错误或持久化。Activities 通过公开 MCP 做只读发现，无 API key，且不展示无币种价格或 click-off link；Hotel 首期仅在 Shared PLAN/REPLAN 中以 snapshot/run-bound `hotel.search` 进行实时搜索和方案比较，模型只可传 `destinationId`，日期、房间/住客数、币种和偏好由服务端推导。酒店 provider 在 task 接受时由服务端配置选择并持久化，不自动 fallback 或混合来源。Ground 分为 `PlaceResolver`、`NavigationProvider`、`TransitJourneyProvider`、`MobilityOfferProvider`，关键词 POI 候选和路线均由 server-owned TripPlace/run context 解析。任何缺失或不可信 provider 都返回 `UNAVAILABLE`，task 安全完成为 `COMPLETED_WITH_GAPS`，不伪造数据；sandbox/fixture 只用于测试。 | 浏览器直连 provider、将 sandbox 当实时库存、供应商 deep link/订单/支付、自动 booking；Nuitee 的 MCP server、Places、Price Index、Prebook/Book 不在本 MVP；Viator MCP 没有公布配额/SLA，不应被宣传为稳定库存或预订能力；公共交通实时和租车在 provider/商业条款 spike 前不承诺覆盖。 |
